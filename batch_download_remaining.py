#!/usr/bin/env python3
"""
Batch download remaining cities using the corrected approach
"""
import json
import requests
import time
...
def download_remaining_boundaries():
    """Download boundaries for cities that don't have them yet"""
    print("🌍 Downloading boundaries for remaining cities...")
    print("=" * 70)
    ...
    # Check which cities don't have boundary files
    missing_cities = []
    ...
    print(f"Found {len(missing_cities)} cities without boundary files:")
    for city in missing_cities:
        print(f"  - {city['name']}, {city['country']} ({city['id']})")
    
    if not missing_cities:
        print("✅ All cities already have boundary files!")
        return
    
    print(f"\nStarting download of {len(missing_cities)} cities...")
    
    downloader = BatchBoundaryDownloader()
    success_count = 0
    failed_cities = []
    ...
    for i, city in enumerate(missing_cities, 1):
        ...
        city_name = city['name']
        country = city['country']
        expected_coords = city['coords']
        
        print(f"\n{i:2d}/{len(missing_cities)}. {city_name}, {country}")
    ...
    print(f"\n📊 Results: {success_count}/{len(missing_cities)} cities successfully downloaded")
