#!/usr/bin/env python3
"""
Improved boundary downloader with better search specificity to get correct cities
"""
import json
import requests
import time
...
class CorrectBoundaryDownloader:
    def __init__(self):
    ...
    def download_city_boundary(self, city_name, country, expected_coords):
        ...
        print(f"\n🌍 Downloading improved boundary for {city_name}, {country}")
...
def download_correct_boundaries():
    """Download correct boundaries for the 11 problematic cities"""
    print("🔧 Downloading correct boundaries for problematic cities...")
    print("=" * 70)
    
    # Load cities database for coordinates
    ...
    downloader = CorrectBoundaryDownloader()
    success_count = 0
    
    for i, city_id in enumerate(problem_cities, 1):
        if city_id not in city_info:
            print(f"{i:2d}/11. ❌ City {city_id} not found in database")
            continue
        ...
        print(f"{i:2d}/11. {city_name}, {country}")
        ...
        try:
            ...
            if geojson:
                ...
                # Backup current file
                backup_filename = f"{city_id}-approximated-backup.geojson"
                if Path(filename).exists():
                    with open(filename, 'r') as f:
                        current_data = json.load(f)
                    with open(backup_filename, 'w') as f:
                        json.dump(current_data, f, indent=2)
                    print(f"   📁 Backed up approximated version to {backup_filename}")
    ...
    print(f"\n📊 Results: {success_count}/11 cities got real boundaries")
    print(f"Remaining {11-success_count} cities still have approximated boundaries")

if __name__ == "__main__":
    download_correct_boundaries()