#!/usr/bin/env python3
"""
Download real boundaries for batch 2 - remaining cities with small files
"""
import json
import requests
import time
...
def download_batch_2():
    """Download real boundaries for batch 2 cities with small files"""
    print("🔄 Downloading real boundaries for batch 2 cities...")
    print("=" * 70)
    ...
    # All remaining small files
    batch2 = small_file_cities
    
    print(f"Found {len(batch2)} cities for batch 2")
    for city_id in batch2:
    ...
    for i, city_id in enumerate(batch2, 1):
        info = city_info[city_id]
        city_name = info['name']
        country = info['country']
        expected_coords = info['coords']
        
        print(f"\n{i:2d}/{len(batch2)}. {city_name}, {country}")
    ...
    print(f"\n📊 Batch 2 Results: {success_count}/{len(batch2)} cities successfully downloaded")
